...
public class ClientHandler extends Thread {
    ...
    private void showCardInfo(Object obj) throws IOException, ClassNotFoundException {
        ...
        int ind = bnk.findMetroCard(op.getCardNumber());
        if (ind >= 0) {
            MetroCard card = bnk.getStore().get(ind);
            os.writeObject(card.getFullInfo());
            os.flush();
        } else {
            os.writeObject("Картка з серійним номером " + op.getCardNumber() + " не знайдена.");
            os.flush();
        }
    }
